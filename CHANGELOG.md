...
- Added durable Verified Changes for MCP, catalog, agentic-os config,
  ...
  rejection, deterministic pre-write backup linkage, activation-aware
  verification, and rollback that refuses later target edits even after an
  interrupted apply.
...
- Unified local and remote WebView HTTP transport in Rust, preserved real HTTP
  status and structured error bodies, restored authenticated remote event
  polling without exposing Keychain bearer tokens to JavaScript, and added
  immediate startup-failure state.
...
- Fixed packaged Python staging to copy the uv-managed standalone runtime
  ...
  absolute runtime symlinks, repo path leaks, bytecode caches, and false
  relocation checks. The packaged daemon disables bytecode writes so app
  launch does not invalidate its resource seal.
...
- 877 Python tests, Ruff, all Web JavaScript syntax checks, and 33 Rust tests
...
- The arm64 `.app` passed bundled-Python execution and strict deep codesign
  verification before and after actually running its bundled daemon, clean tray
  Quit cycles, crash-orphan termination, and relaunch recovery. Developer ID
  signing, notarization, DMG publication, and updater delivery remain external
  release work.
