...
The static no-build UI is `apps/web/index.html`, `apps/web/styles.css`, `apps/web/api.js` (endpoint map), `apps/web/app.js` (shell), and `apps/web/ui/*.js` feature modules (editors, dashboard, dual-track P34–P39 surfaces). Desktop shell: `apps/desktop/` (Tauri). Phase contracts live in `specs/` (001–059); implementation plans live in `docs/superpowers/plans/`. Example harness registry data is in `examples/agents.toml`. README phase table is canonical for P0–P33; P34–P39 dual-track specs are `054`–`059`.
