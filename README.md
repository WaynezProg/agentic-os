...
**Main (2026-06-07):** P11.5 packaged macOS app + P12 remote access adapter + **P12.5** Keychain/iOS companion + **P13** remote approval loop + **P14** remote token lifecycle + **P15** remote HTTPS hardening + **P16** catalog patch UI + **P17** harness config patch UI **complete**. Harness tab supports path patch editing with dry-run `base_mtime` locking in local mode; remote mode stays read-only until P25.
